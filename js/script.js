...
document.addEventListener('DOMContentLoaded', function() {
    ...
    whatsappButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            e.preventDefault();
            const phoneNumber = '201016809096'; // Add country code
            const message = 'مرحباً، أريد الاستفسار عن خدمات مكافحة الحشرات';
            openWhatsApp(phoneNumber, message);
        });
    });

    // Loading screen (if added)
    function hideLoader() {
    ...
    backToTopButton.style.cssText = `
        ...
        width: 50px;
        height: 50px;
        background: linear-gradient(135deg, #2DBDE8, #8BC34A);
        color: white;
        border: none;
        border-radius: 50%;
        cursor: pointer;
        font-size: 18px;
        box-shadow: 0 4px 12px rgba(45, 189, 232, 0.3);
        opacity: 0;
        visibility: hidden;
    ...
    console.log('Aqua Control website loaded successfully!');
});
